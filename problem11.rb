...
GRID = requirements.scan(/^\d{2}.*$/).map { |line| line.scan(/\d+/).map(&:to_i) }

# Walk the grid and yields (direction, n-tuple) pairs
def walk_grid(g, n)
...
def fetch_tuple(g, x, y, dirx, diry, n)
  (1..n).map { |i| g[y+i*diry][x+i*dirx] || 0 }
end

def find_greatest_products(g, n)
  best = {
    horizontal: [0, nil],
    vertical: [0, nil],
    diagonal: [0, nil]
  }
  walk_grid(g, n).each do |dir, tuple|
    prod = tuple.reduce(:*)
    old_best, _ = best[dir]
    if prod > old_best
      best[dir] = [prod, tuple]
...
puts find_greatest_products(GRID, 4).inspect
